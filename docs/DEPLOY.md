...
> 说明：本文档里出现的 `/fpa/`、`fpa-next.service`、`/etc/fpa/` 等，都是**服务器上仍然存在的遗留部署标识**（上一代站点），不是本项目现在的名字。
...
老库的数据搬迁、审计触发器的 DEFINER 重建、以及 `migrate.py verify` 的历史 checksum 漂移说明，
都挪到了 [`docs/LEGACY_DB_MIGRATION.md`](LEGACY_DB_MIGRATION.md)。
