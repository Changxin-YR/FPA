...
必须能解析出 `__yuxin_load_by_id__(tx, *, scope, record_id)`**，否则抛 `INTERNAL_ERROR`。

这个属性只有两处会出现：

* `kernel/runner.py`（读取方，三处 `getattr`）；
* `tools/{runner,web,agent}_e2e.py`（**夹具里手动补挂** `PondService.create.__yuxin_load_by_id__ = ...`）。

于是出现过这样的形态：`backend/yuxin/domains/` 下一处都没有，**生产路径上写能力全部
...
def _master_data_registry():
    ...
    仍然应当是确定的。`yuxin.domains.master_data.capabilities` 的导入链只依赖内核与
...
def _loaders() -> tuple[list[tuple[str, Any]], bool]:
    ...
    return (
        [
            (spec.name, getattr(spec.handler, "__yuxin_load_by_id__", None))
...
def test_every_write_capability_declares_a_reload_function() -> None:
    ...
    assert composition_root_ok, (
        ...
        "请先修好组合根再跑："
        "python -c \"import sys;sys.path.insert(0,'backend');import yuxin.bootstrap as b;b.load_all()\""
    )
    ...
    assert missing == [], (
        ...
        "'我们调用了 INSERT'。请在域内给写能力的处理器挂上 `__yuxin_load_by_id__`"
...
def test_domain_guard_passes_for_master_data() -> None:
    ...
    from yuxin.domains.master_data.capabilities import assert_reload_wired
