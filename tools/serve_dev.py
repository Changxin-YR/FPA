...
`yuxin.wsgi` 里已经有一个模块级 `app`，但它是给 WSGI 服务器用的；而本项目文档里写的
`gunicorn -w 4 "yuxin.wsgi:app"` 在 **Windows 上不可用** —— gunicorn 依赖 `fcntl`。
本机是 Windows，所以用 `waitress` 起**同一个 app**，不另造装配路径
（装配只有一处：`yuxin.factory.build_app()`）。
...
def main(argv: list[str]) -> int:
    port = int(argv[1]) if len(argv) > 1 else int(os.environ.get("YUXIN_DEV_PORT", DEFAULT_PORT))
    host = os.environ.get("YUXIN_DEV_HOST", "127.0.0.1")
