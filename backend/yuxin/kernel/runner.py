...
from yuxin.kernel.errors import DomainError, ErrorCode, forbidden, unauthenticated
...
from yuxin.kernel.invariants import _EXCLUDE_ID_KEY
...
class CapabilityRunner:
    ...
    def _replay(
        ...
        # `__yuxin_read_by_id__`，而能力声明上挂的是 `__yuxin_load_by_id__`，
        # 于是 callable() 为假、静默退化成只返回 {"id": ...} ——
        # 回放数据看起来"有"，但来源不可追溯。一个字母的差别，无报错、无崩溃。
        raw_loader = getattr(spec.handler, "__yuxin_load_by_id__", None)
    ...
    def _maybe_load_before(
        ...
        读取方式复用服务在 `__yuxin_load_by_id__` 上声明的函数，避免 runner 自己写 SQL
        （架构约束：runner 不碰 SQL）。
        """
        raw_loader = getattr(spec.handler, "__yuxin_load_by_id__", None)
    ...
    def _write_audit(
        ...
        分支忘了设置它"的可能。
        """
        from yuxin.kernel.audit import AuditEvent
