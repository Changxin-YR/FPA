...
`tools/master_data_e2e.py`、`tools/cost_e2e.py`（外加 `yuxin/settings.py::from_env()`
的 `mysql` 段）。五处描述同一件事，必然有一处先改。

更要紧的是：**业务域不允许自己连库**（`tests/test_architecture.py` 强制"pymysql 只能
出现在 `yuxin.kernel.uow`"），但 `Field.dynamic_choices` 这类东西确实需要"读几行数据"
...
## 与 `yuxin/settings.py` 的关系（尚未收敛，已知）

`yuxin/settings.py::from_env()` 也拼了一份 `mysql` 字典（默认值与本模块一致：host
`127.0.0.1`、port `3306`、user `yuxin`、database `yuxin`）。两者应当合并为
...
from yuxin.kernel.uow import ConnectionConfig, UnitOfWork
