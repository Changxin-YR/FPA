...
class Settings:
    ...
    #:
    #: 为什么需要这个配置项：Harness 子进程只有拿到这份 patch 才会**挂载 渔芯业务工具**
    #: 并禁用内建工具。不给它，子进程就退回 Harness 出厂形态 —— 模型手里没有业务工具，
    #: 却带着能读文件、能发任意 HTTP 的工具，于是它自己去"手工拼 HTTP 打后端网关"
    #: （实测就是模型那句自述：「本会话里我没有挂载 渔芯的业务工具」）。
    #: 空串时按仓库内的 `agent-runtime/cordis.patch.yml` 推导（见 `yuxin.harness.session`）。
    agent_harness_patch: str = ""
    #: dsh profile 名（env `AGENT_PROFILE`），默认 `sdk` —— 与 SDK 自己的默认值一致，
    #: 插件包按 `docs/DEVELOPMENT.md` §5.3 装在 `<DSH_HOME>/profiles/sdk/node_modules/@yuxin/` 下。
    ...
    #: 端口**，而 `yuxin.wsgi` 听的就是 `PORT`。放在这里，"子进程该往哪回调"与"服务听在哪"
    ...
    def from_env(cls, env: dict[str, str] | None = None) -> Settings:
        ...
        mysql = {
            ...
            "user": (values.get("MYSQL_USER") or "yuxin").strip(),
            "password": mysql_password,
            "database": (values.get("MYSQL_DATABASE") or "yuxin").strip(),
        }
        ...
        origins = tuple(
            item.strip()
            for item in (values.get("YUXIN_CORS_ORIGINS") or "").split(",")
            if item.strip()
        )
