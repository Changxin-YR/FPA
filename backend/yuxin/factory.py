...
`yuxin.bootstrap` 补齐之后，"能力声明"有了装载处；但**真实服务进程**仍然没有装配入口：
...
所以本文件把装配也收敛成**一处**：`yuxin.bootstrap.load_all()` 拿注册表，
其余依赖按同一份环境变量装配。

## 用法

    # 真实进程（gunicorn / wsgi）
    gunicorn -w 4 "yuxin.wsgi:app"
    ...
    from yuxin.factory import assemble
    deps = assemble()                      # deps.registry / runner / access / gateway …

## 一条刻意的边界

本模块**不 import 任何业务域**。`web/` 与装配层都不该知道"有哪些域"——
域的发现是 `yuxin.bootstrap` 的职责（它按目录发现 `domains/*/capabilities.py`）。
这里只调用 `load_all()`，于是"新增一个域"不需要修改本文件。

至于"其余依赖从哪里来"：`yuxin.domains.access` 提供的 `AccessService` /
...
from yuxin.kernel.capability import REGISTRY, Registry
...
from yuxin.web.agent_turn import (
...
class Assembled:
    ...
    而不是从 `app.config` 里摸（`app.config["YUXIN_RUNNER"]` 那种写法把"装配结果"变成
    了运行期才知道的字符串键）。
    """
...
def assemble(
    *,
    ...
    if registry is None:
        # 延迟 import：`yuxin.bootstrap` 会遍历并 import 全部域，放在函数里可以让
    ...
    # `routes_agent` 也在等 `app.config["YUXIN_AGENT_TURN"]`，但**全仓没有任何代码实例化它**
    # —— 生产路径（`yuxin.wsgi:app`）因此静默地什么都不装：应用启动正常、全部 e2e 全绿，
...
def build_app(
    *,
    ...
    `registry=None`（默认）→ 走组合根 `yuxin.bootstrap.load_all()`；
    ...
    app.config["YUXIN_AGENT_GATEWAY"] = deps.gateway
    deps.app = app
    return app
