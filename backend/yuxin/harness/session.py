...
#: 已安装副本的相对路径（`<DSH_HOME>/profiles/<profile>/node_modules/@yuxin/dsh-biz-tools/`）。
_INSTALLED_PATCH_RELPATH = ("profiles", "sdk", "node_modules", "@yuxin", "dsh-biz-tools", "cordis.patch.yml")
...
def resolve_harness_patch(settings: Settings) -> str:
    ...
    3. `<DSH_HOME>/profiles/<profile>/node_modules/@yuxin/dsh-biz-tools/cordis.patch.yml`
...
class HarnessSessionManager:
    ...
    def _create_session(self, *, gateway_url: str, context_token: str) -> Any:
        ...
        if gateway_url:
            env["YUXIN_AGENT_GATEWAY_URL"] = gateway_url
        if context_token:
        ...
        # 一条都没生效：模型手里没有任何 渔芯业务工具，却留着 `tool-web`（能发任意 HTTP）、
        # `tool-fs`（能读文件）、`tool-pwsh`（能执行命令）。实测后果就是模型自己的交代：
        #   「本会话里我没有挂载 渔芯的业务工具，所以是直接用系统给的 X-Agent-Context 凭据
