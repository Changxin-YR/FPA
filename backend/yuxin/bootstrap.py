...
`yuxin/domains/*/capabilities.py` 并逐个 import。新增一个业务域因此不需要修改
任何共享文件——这也是让剩下的五个域能并行开工、而不在同一个 import 列表上
互相冲突的前提。

## `load_all()` 的契约

1. 发现 `yuxin/domains/**/capabilities.py`，导入顺序按名字排序（稳定 → 可复现）；
...
域声明模块直接 `from yuxin.kernel.capability import REGISTRY` 并在模块底部调用
