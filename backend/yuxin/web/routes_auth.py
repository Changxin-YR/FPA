...
from yuxin.web.context import client_ip, current_request_id, user_agent
from yuxin.web.security import (
...
def register_auth_routes(app: Flask) -> None:
    def _access() -> AccessService:
        return app.config["YUXIN_ACCESS"]
    ...
    def logout() -> Response:
        from yuxin.web.security import SESSION_COOKIE
    ...
    def me() -> Response:
        from yuxin.web.app import _current_actor  # 局部导入避免循环依赖

        actor = _current_actor()
        # `_current_actor` 返回的是执行器视角的 ActorView（只有权限码），
        # 而这里要给前端展示姓名等信息，所以再查一次。这个"查两次"是有意的：
        # 执行路径不依赖展示字段，展示路径不依赖执行细节。
        detail = _access().resolve_session(request.cookies.get("yuxin_session", ""))
    ...
    def csrf_token() -> Response:
        ...
        **别把这条注释当成"以后可以放宽"**：把登录所需的令牌预先发给匿名访客，
        ...
        而用户会以为"系统坏了"。
        """
        from yuxin.web.security import CSRF_COOKIE, SESSION_COOKIE
