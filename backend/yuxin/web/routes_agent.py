...
_CTX_SALT = b"yuxin-agent-context-v1"
...
def register_agent_routes(app: Flask) -> None:
    def _gateway() -> Any:
        return current_app.config["YUXIN_AGENT_GATEWAY"]
    ...
    def _actor_from_context_token() -> tuple[Any, dict[str, Any]]:
        ...
        **不来自请求体里的任何字段**。模型即使被注入提示词写了
        ...
        """
        access = current_app.config["YUXIN_ACCESS"]
        ...
        return jsonify(
            {
                ...
                "request_id": current_app.config.get("YUXIN_REQUEST_ID", ""),
            }
        )
    ...
    def agent_tool_call(tool_name: str) -> Response:
        ...
        """
        from yuxin.web.context import current_request_id
    ...
    def agent_turn() -> Response:
        ...
        handler = current_app.config.get("YUXIN_AGENT_TURN")
        if handler is None:
            raise DomainError(
                ErrorCode.AGENT_UNAVAILABLE, "智能助手未启用，请联系管理员"
            )

        from yuxin.web.app import _current_actor
    ...
    def agent_turn_stream() -> Response:
        ...
        * 生成器与**非流式 handler 共用**同一套令牌签发、提示词拼装与结果整形
          （`yuxin.web.agent_turn` 里那一份），这里不做第二份实现；
        * 异常**不**在这里改写成 HTTP 状态码：响应已经开始流式后改状态码是不可靠的。
        ...
        stream_factory = current_app.config.get("YUXIN_AGENT_TURN_STREAM")
