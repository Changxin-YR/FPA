...
# 内核在 `Capability.__post_init__` 里 `setattr(handler, "__yuxin_load_by_id__", loader)`。
# 让五个域各自手写那个属性名字符串，等于把"内核与执行器之间的内部约定"复制五份——
# 内核那一份已经因为一次笔误（`__yuxin_read_by_id__`）让幂等回放静默退化过。
#
...
#     `setattr(handler, "__yuxin_load_by_id__", loader)`）。属性名是内核与执行器之间的内部
