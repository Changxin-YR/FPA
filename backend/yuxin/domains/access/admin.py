...
实测就是这条断言把初稿拦下来的：
`audit.log.list: domain='audit' 但声明在 yuxin.domains.access`。
"""
...
from yuxin.kernel.fields import Choice, RefTarget, f_enum, f_int_list, f_str, f_str_list, text
from yuxin.kernel.password import hash_password, password_problem, verify_password
...
class AdminService:
    ...
    def _check_scopes_active(tx: UnitOfWork, scope_ids: list[int]) -> None:
        ...
        """
        from yuxin.kernel.scope import SCOPE_COLUMN, ScopeType
...
def confirmation_labels(tx: UnitOfWork, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    契约见 `yuxin.kernel.capability.Capability.confirmation_labels`：
