...
from yuxin.kernel.fields import (
    Choice,
    f_date,
    f_enum,
    f_int,
...
def _category_choices(_current: str | None = None) -> tuple[Choice, ...]:
    ...
    **降级的方向是从"看得见"退到"看不见"，而不是从"校验"退到"放行"。**
    ...
    驱动只能出现在 `yuxin.kernel.uow`（`tests/test_architecture.py` 强制）。域里自建
    连接会同时违反两条约束：**依赖方向**，以及"连接配置只有一处"——在补齐本段之前，
    仓库里有 5 份各自从环境变量拼同一份配置的代码。现在配置与连接都来自
    `yuxin.kernel.uow_factory`，本函数只表达"查哪些行"。
    """
    from yuxin.kernel.uow_factory import query_rows
...
# （`CostWriteService.create_entry`），`__yuxin_load_by_id__` 必须挂在同一个函数
# 对象上；装饰器会让"能力声明里的 handler"与"挂载回读函数的对象"变成两个不同的
# 东西，而 `runner._resolve()` 只认前者。
# ---------------------------------------------------------------------------

CostWriteService.create_entry.__yuxin_load_by_id__ = CostEntryService.load_entry  # type: ignore[attr-defined]
CostWriteService.confirm_entry.__yuxin_load_by_id__ = CostEntryService.load_entry  # type: ignore[attr-defined]
CostWriteService.submit_entry.__yuxin_load_by_id__ = CostEntryService.load_entry  # type: ignore[attr-defined]
CostWriteService.archive_entry.__yuxin_load_by_id__ = CostEntryService.load_entry  # type: ignore[attr-defined]
CostWriteService.close_period.__yuxin_load_by_id__ = CostEntryService.load_period  # type: ignore[attr-defined]
