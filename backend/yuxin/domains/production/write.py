...
from yuxin.kernel.fields import f_enum, f_int, f_num, f_ref, f_str, text
...
def _md():
    ...
    """
    try:
        from yuxin.domains.master_data.service import (
            lookup_area,
            lookup_areas,
            lookup_material,
            lookup_materials,
...
def _warehouse_entry_points():
    ...
    """
    try:
        from yuxin.domains.warehouse.ledger import (
            apply_movement,
            ledger_source_ref,
            resolve_issue_lot,
        )
...
def _require_cost_record_fact():
    ...
    try:
        from yuxin.domains.cost.entries import (
            LEDGER_FEED,
...
def _cost_category(material_category: str) -> str:
    ...
    """
    try:
        from yuxin.domains.cost.entries import LEDGER_FEED, MATERIAL_CATEGORY_TO_COST
    except Exception as exc:  # noqa: BLE001
        raise DomainError(
            ErrorCode.INTERNAL_ERROR,
...
# ---------------------------------------------------------------------------
# Reload bindings (`__yuxin_load_by_id__`)
#
...
# (`BatchWriteService.create_batch`), so `__yuxin_load_by_id__` must be attached to
...
BatchWriteService.create_batch.__yuxin_load_by_id__ = BatchService.load_batch  # type: ignore[attr-defined]
BatchWriteService.update_batch.__yuxin_load_by_id__ = BatchService.load_batch  # type: ignore[attr-defined]
BatchWriteService.submit_batch.__yuxin_load_by_id__ = BatchService.load_batch  # type: ignore[attr-defined]
BatchWriteService.verify_batch.__yuxin_load_by_id__ = BatchService.load_batch  # type: ignore[attr-defined]
BatchWriteService.close_batch.__yuxin_load_by_id__ = BatchService.load_batch  # type: ignore[attr-defined]

FeedingWriteService.create_feeding.__yuxin_load_by_id__ = FeedingService.load_feeding  # type: ignore[attr-defined]
FeedingWriteService.verify_feeding.__yuxin_load_by_id__ = FeedingService.load_feeding  # type: ignore[attr-defined]

HarvestWriteService.create_harvest.__yuxin_load_by_id__ = HarvestService.load_harvest  # type: ignore[attr-defined]
HarvestWriteService.verify_harvest.__yuxin_load_by_id__ = HarvestService.load_harvest  # type: ignore[attr-defined]
