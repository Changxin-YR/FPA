...
## `__yuxin_load_by_id__` 必须显式绑定（本域第一条纪律）

`kernel/runner.py::_reload_after` 要求"任何返回 `resource_id` 的写能力，其 handler
必须能解析出 `__yuxin_load_by_id__(tx, *, scope, record_id)`"，否则抛
...
from yuxin.kernel.fields import f_date, f_int, f_num, f_ref, f_str, text
...
# （`PurchaseOrderWriteService.create_order`），`__yuxin_load_by_id__` 必须挂在
