...
from fpa.kernel.errors import DomainError, ErrorCode, not_found, validation
...
def _enrich_order(tx: UnitOfWork, row: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    from fpa.domains.master_data.lookup import lookup_partners, lookup_ponds
...
def _enrich_delivery(tx: UnitOfWork, row: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    from fpa.domains.production.service import lookup_harvest
...
def _enrich_receivable(tx: UnitOfWork, row: dict[str, Any]) -> dict[str, Any]:
    ...
    from fpa.domains.master_data.lookup import lookup_partners
...
class SalesOrderService:
    ...
    def _present(row: dict[str, Any], permissions: frozenset[str]) -> dict[str, Any]:
        ...
        **不是钱**），所以它按"数量 3 位"量化 —— 用金额规则会把 `100.000` 变成
        ...
        from fpa.kernel.money import money
        from fpa.kernel.money import quantity as quantize_quantity
