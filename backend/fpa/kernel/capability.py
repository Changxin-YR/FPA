...
from fpa.kernel.errors import DomainError, ErrorCode, validation
from fpa.kernel.fields import Field, FieldSet
from fpa.kernel.scope import Scope, ScopePolicy, ScopePredicate
...
class Capability:
    ...
    #: 能解析出 ``__fpa_load_by_id__(tx, *, scope, record_id)``，否则**抛错而不是降级成
    #: 成功**（`docs/WRITE_CONTRACT.md` 规则 2：``executed`` 的含义是"读回来的行确实是
    #: 我们要的样子"，不是"我们调用了 INSERT"）。
    #:
    #: 在本参数出现之前，这个属性**没有主人**：`backend/fpa/domains/` 下一处都没挂，
    ...
    def __post_init__(self) -> None:
        ...
        #
        # 挂载动作放在这里而不是让每个域自己 `Handler.__fpa_load_by_id__ = ...`：
        # 属性名是内核与执行器之间的**内部约定**（`runner._reload_after` 按它取），
        # 让五个域各自手写字符串常量，等于把这个约定变成五处副本。
        # 曾经就因为一处写成 `__fpa_read_by_id__` 而另一处写 `__fpa_load_by_id__`，
        # 导致回放静默退化成只返回 `{"id": ...}`（见 `runner._replay` 的注释）。
        if isinstance(self.loader, NoLoader):
            setattr(self.handler, "__fpa_no_loader__", True)
            ...
            setattr(self.handler, "__fpa_load_by_id__", self.loader)
...
def capability(**kwargs: Any) -> Callable[[Handler], Handler]:
    ...
    def decorate(handler: Handler) -> Handler:
        ...
        setattr(handler, "__fpa_capability__", spec)
...
def _field_annotations(handler: Handler) -> FieldSet:
    ...
    **必须用 `get_type_hints(include_extras=True)`**，不能直接读
    ...
    from fpa.kernel.fields import collect_fields  # 局部导入避免循环
...
def assert_authorized(capability_spec: Capability, permissions: Iterable[str]) -> None:
    from fpa.kernel.errors import forbidden
