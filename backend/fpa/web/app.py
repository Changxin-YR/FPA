...
from fpa.kernel.capability import HttpMethod, Registry
...
from fpa.web.context import bind_request_context, current_request_id, request_meta
from fpa.web.security import CSRF_HEADER, SESSION_COOKIE, require_csrf
...
def create_app(
    *,
    ...
    app.config["FPA_REGISTRY"] = registry
    app.config["FPA_RUNNER"] = runner
    app.config["FPA_ACCESS"] = access
    app.config["FPA_SCOPE_RESOLVER"] = scope_resolver

    _register_middleware(app)

    from fpa.web.routes_auth import register_auth_routes
    ...
    app.config["FPA_AGENT_TURN"] = agent_turn_handler
    ...
    app.config["FPA_AGENT_TURN_STREAM"] = agent_turn_stream_handler
    from fpa.web.routes_agent import register_agent_routes
...
def _make_view(capability_name: str) -> Any:
    ...
    def view(**path_params: Any) -> Response:
        ...
        # 函数体内，启动时不会暴露。
        runner: CapabilityRunner = current_app.config["FPA_RUNNER"]
...
def _current_actor() -> ActorView:
    ...
    **身份只来自 Cookie 里的会话令牌**，不接受任何请求体/请求头声称的 user_id 或 role。
    ...
    """
    access: AccessService = current_app.config["FPA_ACCESS"]
...
def _register_meta_routes(app: Flask) -> None:
    from fpa.web.workflow_meta import actions_payload, resources_payload
    ...
    def meta_capabilities() -> Response:
        ...
        """
        registry_: Registry = current_app.config["FPA_REGISTRY"]
        actor = _current_actor()
