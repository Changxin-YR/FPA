...
## 双提交（double-submit）：为什么是三方比对
    ...
    浏览器 Cookie `fpa_csrf`        <- 服务端写入，JS 可读（非 HttpOnly）
...
**为什么 CSRF 存原值而会话令牌存哈希**——这是刻意的区别，不是疏漏：
    ...
    会话令牌（fpa_session，HttpOnly）-> 服务端只存 sha256，**不可还原**
        它是真正的凭据；库泄露时攻击者拿不到可直接冒用的值
    CSRF 令牌（fpa_csrf，非 HttpOnly）-> 服务端存原值
...
SESSION_COOKIE = "fpa_session"

#: CSRF Cookie 名。非 HttpOnly——前端必须能读它来填请求头。
CSRF_COOKIE = "fpa_csrf"
...
def require_csrf() -> None:
    ...
    access = current_app.config.get("FPA_ACCESS")
