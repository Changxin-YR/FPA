...
`fpa.factory.build_app` 的注释里自己写着：
...
`app.config["FPA_AGENT_TURN"]`，但**全仓没有任何代码实例化那个类**。
`build_app(agent_turn_handler=None)` 的默认值让生产路径（`fpa.wsgi:app`）
...
## 一轮对话的数据流
    ...
    POST /api/v1/agent/turns  (浏览器 + 会话 Cookie + CSRF)
      ...
      -> 子进程里的 `@fpa/dsh-biz-tools` 插件带 `X-Agent-Context` 调
...
#: `gatewayUrl: !!js process.env.FPA_AGENT_GATEWAY_URL ?? ''` **是同一个键** ——
#: 插件就是从子进程环境里读它的，所以这里导出的名字必须与它一致。
GATEWAY_URL_ENV = "FPA_AGENT_GATEWAY_URL"

#: 上下文令牌的环境变量名（同样与 patch 文件里的 `contextToken:` 那行一致）。
CONTEXT_TOKEN_ENV = "FPA_AGENT_CONTEXT_TOKEN"
...
def resolve_gateway_url(settings: Settings, *, port: int | None = None) -> str:
    ...
    2. 推导：`http://127.0.0.1:<port>/api/v1/agent`，端口取 `settings.port`
       （env `PORT`，与 `fpa.wsgi` 听的是同一个值；未配置时用 Flask 默认 5000）。
