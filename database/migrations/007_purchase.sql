...
-- ============================================================================
-- 自我对齐：`cancel_reason` -> `reason`
--
-- 为什么迁移里要有这一段：007 **已经登记过**，而它后来又按 ARCHIVED_DECISIONS
-- R4「描述服从字段表」改过一次——registry §2.11 权威字段表写的是 `reason`
-- （"早期版本叫 `cancellation_reason`，本版统一为 `reason`"），`cancel_reason`
-- 只出现在 §4 #22 的**描述**里。改的是文件，真库的列还叫 `cancel_reason`，
-- 于是 runner 把登记值对不上报成漂移。
--
-- 处置同 004：文件是真话，**真库向文件对齐**，动作写在迁移自身里。
--
-- **可重入**：
--   * 列已是 `reason`（新库）-> `DO 0`，且 CHECK 已由上面的 CREATE TABLE 建好，
--     整段无事发生；
--   * 列还是 `cancel_reason`（早期版本库）-> 改名。MySQL 的 `RENAME COLUMN` 会**自动
--     重写**引用该列的 CHECK 表达式（`chk_purchase_orders_cancel_reason` 由此
--     变成 `chk_purchase_orders_reason`），所以这里只需在改名前删掉旧名那条，
--     再由下面按新名重建 —— 否则会同时存在两条同义约束。
-- ============================================================================
...
SET @ddl = IF(@had_old,
  'ALTER TABLE purchase_orders DROP CONSTRAINT chk_purchase_orders_cancel_reason',
  'DO 0');
...
-- 早期版本库里改名后没有约束（上面显式删了），新库里 CREATE TABLE 已经带了 ——
-- 所以只在"没这条约束"时补上，避免重复定义。
SET @missing = (
  SELECT COUNT(*) = 0 FROM information_schema.TABLE_CONSTRAINTS
   WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = 'purchase_orders'
     AND CONSTRAINT_NAME = 'chk_purchase_orders_reason'
);
SET @ddl = IF(@missing,
  "ALTER TABLE purchase_orders ADD CONSTRAINT chk_purchase_orders_reason CHECK (status <> 'cancelled' OR (reason IS NOT NULL AND reason <> ''))",
  'DO 0');
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
